...
for (const componentName of componentNames) {
  ...
  // Rename the CommonJS build so that it can be imported with
  // vue-resize-element/dist/ComponentName
...
if (process.env.VUE_APP_E2E) {
  ...
  const nodeModulesDir = path.resolve(
    __dirname,
    '../node_modules/vue-resize-element'
  )
  ...
  ncp(
    path.resolve(__dirname, '../dist'),
    path.resolve(__dirname, '../node_modules/vue-resize-element/dist'),
    error => {
      if (error) console.error(error)
    }
  )
  ncp(
    ...
    path.resolve(__dirname, '../node_modules/vue-resize-element/src'),
    error => {
      if (error) console.error(error)
    }
  )
